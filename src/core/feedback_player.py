...
class FeedbackPlayer(QObject):
    def __init__(self, start_sound_path: str | Path | None, stop_sound_path: str | Path | None, parent: QObject | None = None) -> None:
        ...
        self.start_sound_path = self._resolve_sound_path(start_sound_path, BUNDLED_START_SOUND_PATH)
        self.stop_sound_path = self._resolve_sound_path(stop_sound_path, BUNDLED_STOP_SOUND_PATH)
        self._audio_output = QAudioOutput(self)
        self._audio_output.setVolume(1.0)
        self._player = QMediaPlayer(self)
        self._player.setAudioOutput(self._audio_output)

    def _resolve_sound_path(self, configured_path: str | Path | None, bundled_path: Path) -> Path | None:
        if configured_path:
